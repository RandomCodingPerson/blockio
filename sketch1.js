...
function fall() {
    let fall = true;
        for (i = 0; i < blocks.length; i++) {
            ...
            if (touchingBlock || (playerY >= 233.1)) {
                fall = false;
                velocityY = 0;

                if (touchingBlock) {
                    playerY = blocks[i].y - 33.3;
                } else if (playerY >= 233.1) {
                    playerY = 233.1;
                }
            }
        }
...
function coinCollision() {
    for (i = 0; i < coins.length; i++) {
        let charOverCoin = (playerY < coins[i].y) && (coins[i].y - playerY <= 33.3);
        let charUnderCoin = (playerY > coins[i].y) && (playerY - coins[i].y <= 33.3);
        let charLeftOfCoin = (playerX < coins[i].x) && (coins[i].x - playerX < 33.3);
...
function spikeCollision() {
    for (i = 0; i < spikes.length; i++) {
        let charOverSpike = (playerY < spikes[i].y) && (spikes[i].y - playerY <= 33.3);
        let charUnderSpike = (playerY > spikes[i].y) && (playerY - spikes[i].y <= 33.3);
        let charLeftOfSpike = (playerX < spikes[i].x) && (spikes[i].x - playerX < 33.3);
...
function moveOn() {
    for (i = 0; i < finish.length; i++) {
        let charOverFinish = (playerY < finish[i].y) && (finish[i].y - playerY <= 33.3);
        let charUnderFinish = (playerY > finish[i].y) && (playerY - finish[i].y <= 33.3);
        let charLeftOfFinish = (playerX < finish[i].x) && (finish[i].x - playerX < 33.3);
